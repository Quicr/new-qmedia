...
{
    log(quicr::LogLevel::info,
        "[Delegate] Media Connection Closed: " + name);
...
void Delegate::get_queued_messages(std::vector<TransportMessageInfo> &messages_out)
{
    std::lock_guard<std::mutex> lock(queue_mutex);
...
MediaTransport::MediaTransport(const std::string &server_ip, const uint16_t port)
...
void MediaTransport::check_network_messages(std::vector<TransportMessageInfo> &messages_out)
{
